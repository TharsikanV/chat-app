...
const MessagePage = () => {
  ...
  useEffect(()=>{
    if(currentMessage){
      currentMessage.current.scrollIntoView({behavior:'smooth',block})
    }
  })
  ...
  return (
    <div style={{ backgroundImage: `url(${backgroundImage})` }} className='bg-none bg-cover'>
      ...
      <section className='h-[calc(100vh-128px)] overflow-x-hidden overflow-y-scroll scrollbar relative bg-slate-200 bg-opacity-50'>
        ...
        {/**all meassage show here */}
        <div className='flex flex-col gap-2 py-2'>
          {
            allMessage.map((msg,index)=>{
              return(
                <div ref={currentMessage} className={`bg-white p-1 py-1 rounded w-fit ${user._id === msg.msgByUserId ? "ml-auto":""}`}>
